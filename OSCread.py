...
def get_singleCh_file_list(file_list) :
    Empty = True
    single_Ch_filelist = []
    if Empty :
    ...
    if (entry == 0) :
        ...
        sys.exit()
    return single_Ch_filelist, entry
...
def Get_Timing(file_list) :
    single_Ch_filelist, entry = get_singleCh_file_list(file_list)
...
def plot_timing(file_list) :
    ...
    ax1 = plt.subplot(1, 2, 1, title='Time difference', ylabel='sec', xlabel='Evt Num')
    ax2 = plt.subplot(1, 2, 2, title='Norm time', ylabel='sec', xlabel='Evt Num')
    for ax in (ax1, ax2) :
