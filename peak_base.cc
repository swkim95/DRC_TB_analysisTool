...
void peak_base(std::string fileNum, std::string pedNum, enum runMode mode) {
    ...
    if ( mode == runMode::kFull ) {
        c->SaveAs(BASE_DIR+"plots/Peak/png/"+FILE_NAME+"_"+(TString)fileNum+"_full.png");
        c->SaveAs(BASE_DIR+"plots/Peak/svg/"+FILE_NAME+"_"+(TString)fileNum+"_full.svg");
    }
    if ( mode == runMode::kPrompt ) {
        c->SaveAs(BASE_DIR+"plots/Peak/png/"+FILE_NAME+"_"+(TString)fileNum+"_prompt.png");
        c->SaveAs(BASE_DIR+"plots/Peak/svg/"+FILE_NAME+"_"+(TString)fileNum+"_prompt.svg");
    }
    ...
    TFile* validFile = new TFile((TString)(BASE_DIR+"validFiles/Peak/"+FILE_NAME+"_"+(TString)fileNum+"_validation_"+std::to_string(mode)+".root"), "RECREATE");
